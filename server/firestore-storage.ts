...
export class FirestoreStorage implements IStorage {
  ...
  private async _performWithdrawal(userId: string, amount: string, idempotencyKey: string): Promise<WithdrawalResult> {
    try {
      const result = await this.db.runTransaction(async (transaction) => {
        ...
        const totalWinnings = calculateWinnings(transactions);
        const totalWithdrawn = calculateWithdrawn(transactions);
        const availableForWithdrawal = totalWinnings.subtract(totalWithdrawn);

        if (requestedAmount.isGreaterThan(availableForWithdrawal)) {
          return {
            success: false,
            error: "Insufficient winnings available for withdrawal",
        ...
        return {
          ...
          availableForWithdrawal: availableForWithdrawal.subtract(requestedAmount).toString(),
        };
      });
  ...
  async createGame(insertGame: InsertGame): Promise<Game> {
    ...
    // Calculate prize amount: entry fee × max players × (1 - commission rate)
    ...
    const prizeAmount = (totalEntryFees * (1 - commissionRate)).toFixed(2);
