...
export interface IStorage {
  ...
  batchGamePlay(
    userId: string,
    gameType: 'plinko' | 'dice',
    user: User,
    newBalance: string,
    transaction: Omit<InsertTransaction, 'createdAt'>,
    gameResult: InsertPlinkoResult | InsertDiceResult,
    newGamesPlayed: number,
    newGamesWon: number,
    newTotalWinnings: string
  ): Promise<{ transaction: Transaction; gameResult: PlinkoResult | DiceResult }>;
...
export class MemStorage implements IStorage {
  ...
  async batchGamePlay(
    ...
    return {
      ...
      gameResult: resultDoc as PlinkoResult | DiceResult,
    };
  }
...
export class FileStorage implements IStorage {
  ...
  async batchGamePlay(
    ...
    const [users, transactions, plinkoResults, diceResults] = await Promise.all([
      this.loadUsers(),
      this.loadTransactions(),
    ...
    await Promise.all([
      this.saveUsers(users),
      this.saveTransactions(transactions),
      gameType === 'plinko' ? this.savePlinkoResults(plinkoResults) : this.saveDiceResults(diceResults),
    ]);
