...
export function useYahtzeeGameState(gameId: string) {
  return useQuery<YahtzeeGameState>({
    ...
    refetchInterval: (query) => (query.state.data?.game.status === "completed" ? false : 2000), // Auto-stop polling when completed
    enabled: !!gameId,
  });
}
...
export function useCurrentTurn(gameId: string, stopPolling?: boolean) {
  return useQuery<YahtzeeTurn>({
    ...
    refetchInterval: stopPolling ? false : 1000, // Stop polling if game is completed
