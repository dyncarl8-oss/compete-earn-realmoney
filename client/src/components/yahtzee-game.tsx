...
export default function YahtzeeGame({ gameId, onBackToLobby }: YahtzeeGameProps) {
  ...
  const { data: currentTurn } = useCurrentTurn(gameId, isGameCompleted);
  ...
  // Helper functions for dice positioning
  const getCurrentPlayerPosition = (): TablePosition | null => {
    if (!currentPlayer || !currentUserId || !gameState) return null;
    ...
    return currentPlayerPos?.position || null;
  };
  ...
  if (gameStateLoading || !gameState) {
    return (
      <div className="flex items-center justify-center p-8">
        <div className="flex items-center space-x-2">
          <Loader2 className="w-6 h-6 animate-spin text-blue-500" />
          <span className="text-slate-400">Loading Yahtzee game...</span>
        </div>
      </div>
    );
  }
  ...
  // This can happen when Whop user data is still loading
  if (!myPlayerState && whopUser) {
    return (
      <div className="flex items-center justify-center p-8">
        <div className="flex items-center space-x-2">
          <Loader2 className="w-6 h-6 animate-spin text-blue-500" />
  ...
  }, [currentTurn?.userId, gameState?.currentRound, dicePhase]);

  // Generate parked dice positions when phase changes to parked
  useEffect(() => {
    if (dicePhase === 'parked' && currentPlayer && dice.length > 0) {
      const currentPlayerPosition = getCurrentPlayerPosition();
      if (currentPlayerPosition) {
        const positions = computeParkedDicePositions(dice.length, currentPlayerPosition);
        setParkedDicePositions(positions);
      }
    }
  }, [dicePhase, currentPlayer?.user.id, dice.length]);
  ...
  }, [currentTurn?.rollCount, gameId, currentPlayer?.user.id, dice.length, dicePhase]);

  // Update held dice positions when dice are held/unheld
  useEffect(() => {
    if (!currentTurn || !currentPlayer || dicePhase !== 'scattered') return;
    ...
    if (heldDice.length > 0) {
      const heldPositions = computeHeldDicePositions(heldDice.length, currentPlayerPosition);
      setHeldDicePositions(heldPositions);
    } else {
      setHeldDicePositions([]);
    }
  }, [dice.map(d => d.held).join(','), currentPlayer?.user.id, dicePhase]);
